...
| 단계 | 내용 |
|------|------|
| 1. 필터링 | 데이터를 로드한 후, `label` 값(0=정상, 1=스팸)에 따라 데이터를 분리 |
| 2. 데이터 클리닝 및 정규화 | 텍스트 길이 제한, 언어 필터링, 중복 행 제거. `label` 값은 0과 1로 정규화 |
| 3. 샘플링 | 각 `label` 별 데이터 수를 확인하고, 균형 잡힌 샘플을 추출 |
| 4. 데이터 저장 | 샘플링된 데이터와 나머지 데이터를 각각 별도 파일로 저장 |
...
**정제 결과**: 행은 **102,238개**, 정상 데이터: **52,197건**, 스팸 데이터: **49,940건**이 추출이 되었습니다.
